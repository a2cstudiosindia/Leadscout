...
function buildPDFDocument(report: ScanReport, branding?: AgencyBranding): jsPDF {
    const doc = new jsPDF();
...
export function generatePDF(report: ScanReport, branding?: AgencyBranding) {
    const doc = buildPDFDocument(report, branding);
    doc.save(`audit_report_${report.url.replace(/[^a-z0-9]/gi, '_')}.pdf`);
}

export function generatePDFBuffer(report: ScanReport, branding?: AgencyBranding): Buffer {
    const doc = buildPDFDocument(report, branding);
